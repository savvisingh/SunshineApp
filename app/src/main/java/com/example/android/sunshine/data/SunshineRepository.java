...
public class SunshineRepository {
    ...
    private LiveData<WeatherEntry[]> mDownloadedWeatherForecasts;
    ...
    private SunshineRepository(WeatherDao weatherDao,
        ...
        mDownloadedWeatherForecasts = mWeatherNetworkDataSource.getCurrentWeatherForecasts();
    ...
    public LiveData<List<WeatherEntry>> getWeatherForcastList(){
        initializeData();
        Date date = SunshineDateUtils.getNormalizedUtcDateForToday();
        return mWeatherDao.getWeatherForecastList(date);
    }

    public void addWeatherEntry(WeatherEntry weatherEntry){
        mExecutors.diskIO().execute(() -> mWeatherDao.bulkInsert(weatherEntry));
    }
}